...
class NotificationReaderServiceTest {
    ...
    public void should_get_list_notifications_only_with_subscription() {
        ...
        UserIdRequestDto userIdRequestDto = new UserIdRequestDto(UUID.randomUUID());
        userService.addNewUser(userIdRequestDto);
        UUID userTechnicalId = userIdRequestDto.getTechnicalId();

        subscriptionService.addSubscriptionForEvent(eventRepository.findAll().get(0).getEventId(), userTechnicalId);

        //when
        List<NotificationResponseDto> allNotificationsForSubscribedEvents = userService.findAllNotificationsForSubscribedEvents(userTechnicalId);
